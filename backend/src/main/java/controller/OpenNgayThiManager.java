 ...
 */
@WebServlet("/api/open-ngay-thi")
public class OpenNgayThiManager extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public OpenNgayThiManager() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String dateExam = json.optString("dateExam", "");
			System.out.println("dateExam: " + dateExam);
			boolean success = KyThiManagerDAO.updateExamDate(conn, type, dateExam);
			if (!success) {
